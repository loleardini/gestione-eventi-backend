<?php
// Utilita per database e autenticazione
...
// Polyfill per getallheaders() se non esiste
...
// Funzione generica per inviare JSON
...
// Interazione veloce con Supabase REST API in PHP curl
function supabaseRequest($method, $table, $query = '', $data = null) {
    ...
    // curl_close è deprecato da PHP 8.0 in su
    
    return ['code' => $httpCode, 'data' => json_decode($response, true)];
}

// Genera un semplice JSON Web Token indipendente senza librerie esterne
function generateJWT($payload) {
    $header = json_encode(['typ' => 'JWT', 'alg' => 'HS256']);
    $payload['exp'] = time() + (86400 * 7); // Scade tra 7 giorni
...
// Controlla il token nella richiesta, decodifica utente
function authMiddleware() {
    ...
    return $payload; // Ritorna id utente, ruolo, ecc
}
